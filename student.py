...
from consts import *
from tree_search_boxes_helper import Maze
import websockets
...
async def solver(puzzle, solution):
    while True:
        ...
        boxproblem = SearchProblemBoxes(Maze(mapa), mapa.boxes)
        boxtree = SearchTreeBoxes(boxproblem)
        keys = await boxtree.search()
        await solution.put(keys)

async def agent_loop(puzzle, solution, server_address="localhost:8000", agent_name="93127"):
